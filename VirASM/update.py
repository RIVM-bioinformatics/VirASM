...
def update(sysargs):
    ...
    try:
        latest_release = request.urlopen(
            "https://api.github.com/repos/RIVM-bioinformatics/VirASM/releases/latest"
        )
    ...
    if localversion < latest_release_tag_tidied:
        if (
            AskPrompts(
                f"""
There's a new version of VirASM available.
...
Latest version: {color.GREEN + color.BOLD}{latest_release_tag}{color.END}\n""",
            ...
            subprocess.run(
                [
                    ...
                    "install",
                    "--upgrade",
                    f"git+https://github.com/RIVM-bioinformatics/VirASM@{latest_release_tag}",
                ],
                check=True,
            ...
            print(
                f"VirASM updated to {color.YELLOW + color.BOLD}{latest_release_tag}{color.END}"
            )
